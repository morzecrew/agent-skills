...
A rule you have to remember will eventually be forgotten around. A rule a program declines to let past will not.
...
- **Handle empty values deliberately.** Converting `None` to text yields the string `"None"`, which reads as present — so a null field, the conventional way of writing "nothing here yet", counted as a recorded decision and gave every writer a one-token way out of its obligations. Coerce with `(value or "")`, and pin `None`, `False`, `0`, `[]`, `{}`, `""` and whitespace in a test.
